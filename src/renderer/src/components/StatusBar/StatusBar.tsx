...
export const StatusBar = () => {
  ...
  const { isLanguageSelectorOpen: isLangOpen, setLanguageSelectorOpen: setIsLangOpen } = useUIStore();
      ...
      <div className={styles.left}>
        {activeTab ? (
          <>
            ...
            {activeTab.encrypted && (
              <span className={styles.item} title="This tab is encrypted at rest">🔒</span>
            )}
            {cursorPosition && (
        ...
        {activeTab && (
          <>
            <button 
              ...
              onClick={() => (window as any).api.setTabEncrypted(activeTab.id, !activeTab.encrypted)}
            ...
            <button 
               ...
               style={{ cursor: 'pointer', border: 'none', background: 'transparent', padding: '0 8px' }}
            >
              {activeTab.language || 'plaintext'}
            </button>
            <button 
              className={styles.item}
              onClick={() => window.dispatchEvent(new CustomEvent('monaco:format'))}
