...
export const MonacoInstance = ({ tab, isActive }: MonacoInstanceProps) => {
  ...
  const handleEditorDidMount: OnMount = (editor) => {
    ...
    editor.addAction({
      ...
      run: async (ed) => {
        ...
        ed.executeEdits('utility', [{ range: selection, text: num.toString() }]);
      }
    });
  };
  ...
  const handleChange = (value: string | undefined) => {
    ...
    debounceRef.current = setTimeout(() => {
      window.api.updateTab(tab.id, { content: newContent, updatedAt: Date.now() });
    }, 300);
  };

  const theme = settings?.theme === 'light' ? 'notevault-light' : 'notevault-dark';
  ...
  return (
    <Editor
      ...
      options={{
        fontFamily: settings?.editorFontFamily || "'Cascadia Mono', Consolas, monospace",
