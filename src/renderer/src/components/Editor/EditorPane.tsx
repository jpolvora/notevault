...
export const EditorPane = () => {
  ...
  const { diffTabId, setDiffTabId } = useUIStore();
    ...
    <div style={{ flex: 1, position: 'relative', width: '100%', height: '100%' }}>
      {tabs.length === 0 ? (
        ...
        tabs.map(tab => (
          ...
          >
            {diffTabId === tab.id ? (
              <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', background: 'var(--color-bg)' }}>
                <div style={{ padding: '8px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--color-border)' }}>
                  <span style={{ fontSize: '12px', color: 'var(--color-text-muted)' }}>Comparing local with last synced version</span>
                  <button 
                    onClick={() => setDiffTabId(null)}
                    style={{ padding: '4px 8px', borderRadius: '4px', background: 'var(--color-surface-hover)', fontSize: '11px' }}
                  >
                    Close Diff
                  </button>
                </div>
