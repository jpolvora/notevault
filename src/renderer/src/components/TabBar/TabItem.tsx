...
export const TabItem = ({ tab, isActive, onClick }: TabItemProps) => {
  const { removeTab, togglePin, updateTab, toggleEncryption, setColor } = useTabStore();
  const { setDiffTabId } = useUIStore();
  ...
  const menuOptions = [
    { label: tab.pinned ? 'Unpin Tab' : 'Pin Tab', onClick: () => togglePin(tab.id) },
    { label: tab.encrypted ? 'Remove Encryption' : 'Encrypt Tab', onClick: () => toggleEncryption(tab.id) },
    { label: 'Rename Tab', onClick: () => setIsRenaming(true) },
    { label: 'Show changes since last sync', onClick: () => setDiffTabId(tab.id), divider: true },
    { 
        label: 'Export Tab...', 
    ...
    { label: 'Orange', color: 'orange', onClick: () => setColor(tab.id, 'orange') },
    { label: 'Yellow', color: 'yellow', onClick: () => setColor(tab.id, 'yellow') },
    { label: 'Green', color: 'green', onClick: () => setColor(tab.id, 'green') },
  ...
  return (
    <>
      <div
        ...
        title={tab.encrypted ? 'Encrypted (Right-click for options)' : 'Click to select, right-click for options'}
      >
        <div className={styles.itemGroup}>
          ...
          {tab.encrypted && (
            <span className={styles.lockIcon} title="Encrypted Tab">🔒</span>
          )}
          {isRenaming ? (
            <input
              ref={inputRef}
        ...
        {!tab.pinned && (
          <button className={styles.closeBtn} onClick={handleClose}>
            <svg width="8" height="8" viewBox="0 1 10 10"><path d="M0,0l10,10M10,0L0,10" stroke="currentColor" strokeWidth="1.5"/></svg>
          </button>
        )}
      </div>
