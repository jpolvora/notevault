...
export function setupCryptoIpc() {
  ...
  ipcMain.handle('crypto:setEncrypted', async (_, { tabId, encrypted }: { tabId: string; encrypted: boolean }) => {
  ...
  ipcMain.handle('crypto:changePassphrase', async (_, { newPassphrase }: { newPassphrase: string }) => {
    if (!cryptoService.isUnlocked()) return { ok: false, error: 'Vault is locked' };
  ...
  ipcMain.handle('crypto:hash', (_, { text, algorithm }: { text: string; algorithm: string }) => {
    const crypto = require('node:crypto');
    return crypto.createHash(algorithm).update(text).digest('hex');
  });

  ipcMain.handle('utility:random', (_, { min, max }: { min: number; max: number }) => {
