...
class StorageService {
  ...
  importTabs(newTabs: Tab[], merge: boolean) {
    if (merge) {
      const currentTabs = this.getTabs();
      const existingIds = new Set(currentTabs.map((t) => t.id));
      const filteredNewTabs = newTabs.filter((t) => !existingIds.has(t.id));
      this.setTabs([...currentTabs, ...filteredNewTabs]);
    } else {
