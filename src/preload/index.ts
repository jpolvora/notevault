...
const api = {
  ...
  exportFile: (content: string, label: string, format: string) =>
    ipcRenderer.invoke("tab:exportFile", content, label, format),
  exportAllZip: () => ipcRenderer.invoke("tabs:exportAllZip"),
  importAll: (merge: boolean) => ipcRenderer.invoke("tabs:importAll", merge),
  clearAllTabs: () => ipcRenderer.invoke("tabs:clearAll"),
  getHash: (text: string, algorithm: string) =>
    ipcRenderer.invoke("crypto:hash", { text, algorithm }),
  getRandom: (min: number, max: number) =>
    ipcRenderer.invoke("utility:random", { min, max }),
  ...
  setTabEncrypted: (tabId, encrypted) =>
    ipcRenderer.invoke("crypto:setEncrypted", { tabId, encrypted }),
  ...
  backupToCloud: () => ipcRenderer.invoke("sync:backupToCloud"),
  listCloudBackups: () => ipcRenderer.invoke("sync:listCloudBackups"),
  restoreFromCloud: (fileId: string) =>
    ipcRenderer.invoke("sync:restoreFromCloud", fileId),
  testGoogleConfig: (clientId: string, clientSecret: string) =>
    ipcRenderer.invoke("google:test-config", clientId, clientSecret),
