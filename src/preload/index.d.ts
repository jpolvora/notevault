...
declare global {
  interface Window {
    ...
    api: {
      ...
      importFile: (
        path: string,
      ) => Promise<{ content: string; label: string; language: string } | null>;
      exportFile: (
        content: string,
        label: string,
        format: string,
      ) => Promise<boolean>;
      exportAllZip: () => Promise<boolean>;
      importAll: (merge: boolean) => Promise<boolean>;
      clearAllTabs: () => Promise<boolean>;
      ...
      backupToCloud: () => Promise<string>;
      listCloudBackups: () => Promise<
        { id: string; name: string; createdTime: string }[]
      >;
      restoreFromCloud: (fileId: string) => Promise<number>;
      testGoogleConfig: (
        clientId: string,
        clientSecret: string,
      ) => Promise<boolean>;
